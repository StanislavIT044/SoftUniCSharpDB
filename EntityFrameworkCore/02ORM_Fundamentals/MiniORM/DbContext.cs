...
namespace MiniORM
{
	public abstract class DbContext
	{
			...
			where TCollection : class, new()
		{
			...
			foreach (var entity in dbSet)
			{
				...
				TCollection[] navigationEntities = navigationDbSet
					.Where(navigationEntity => primaryKey.GetValue(navigationEntity).Equals(primaryKey))
					.ToArray();
			...
			where TEntity : class
		{
			...
			IEnumerable<string> columns = GetEntityColumnNames(table);

			string tableName = GetTableName(table);

			var fetchedRows = this
				.connection
